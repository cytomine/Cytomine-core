...
class ImageScoreService extends ModelService {
    ...
    def statsReport(Project project) {
        ...
        def rows = new ArrayList<LinkedHashMap<String,String>>()

        imageInstanceList.each { image ->
            userList.each { user ->
                LinkedHashMap<String,String> row = new LinkedHashMap<>()
                row.put("imageId", String.valueOf(image.id))
                row.put("instanceFilename", image.baseImage.filename)
