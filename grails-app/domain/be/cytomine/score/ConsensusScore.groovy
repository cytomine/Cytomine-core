...
class ConsensusScore extends CytomineDomain implements Serializable {
    ...
    void checkAlreadyExist() {
        ConsensusScore.withNewSession {
            if (imageInstance && scoreValue) {
                ...
                if (scoreAlreadyExist && (scoreAlreadyExist.id != id)) {
                    throw new AlreadyExistException("Consensus Score $scoreValue already exist in image $image !")
                }
            }
        }
    }
     ...
     */
    static ConsensusScore insertDataIntoDomain(def json, def domain = new ConsensusScore()) {
    ...
    public CytomineDomain container() {
        return imageInstance.container()
    }
}
